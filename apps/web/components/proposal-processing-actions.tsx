...
export function ProposalProcessingActions({
  ...
  const [status, setStatus] = useState(
    failedJobCount > 0
      ? `실패한 자동 처리 작업 ${failedJobCount}건을 다시 실행할 수 있습니다`
      : "현재 다시 실행할 자동 처리 작업은 없습니다",
  );

  if (failedJobCount < 1) {
    return (
      <div className={styles.panel}>
        <p className={styles.label}>자동 처리 상태</p>
  ...
  const handleRerun = async () => {
    ...
    try {
      ...
      setStatus("자동 처리 작업을 다시 실행 대기열에 올렸습니다");
      router.refresh();
    } catch (error) {
      setStatus(
        error instanceof Error && error.message === "proposal-rerun-access-expired"
          ? "편집 권한 또는 Access 세션이 끊겨 자동 처리 작업을 다시 실행하지 못했습니다. 다시 로그인한 뒤 시도해 주세요"
          : "자동 처리 작업을 다시 실행하지 못했습니다. 권한과 연결 상태를 확인해 주세요",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className={styles.panel}>
      ...
      <button
        ...
        onClick={handleRerun}
      >
        {submitting ? "다시 실행 중..." : "자동 처리 다시 실행"}
      </button>
    </div>
  );
}
