...
import { uploadEditorialV2ImageForRevision } from "@/lib/server/editorial-v2/workflow";
...
export async function POST(
  ...
  try {
    ...
    const result = await uploadEditorialV2ImageForRevision(
      revisionId,
      file,
      identity.email,
    );
    ...
    console.error("Failed to upload v2 editorial image by revision", error);
    const rawDetail =
