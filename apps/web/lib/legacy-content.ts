...
export const getLegacyPublishedArticles = cache(async (): Promise<
  PublishedArticleSummary[]
> => {
...
export const getLegacyFeaturedArticle = cache(async () => {
  const articles = await getLegacyPublishedArticles();
  return articles.find((article) => article.featured) ?? articles[0] ?? null;
});

export const getLegacyLatestArticles = cache(
...
export const getLegacyArticleBySlug = cache(
...
export const getLegacyPublishedArticleSourceBySlug = cache(
  async (slug: string) => {
    const sources = await readAllArticleSources();
    const source = sources.find((entry) => entry.article.slug === slug);
...
export const getLegacyRelatedArticles = cache(
